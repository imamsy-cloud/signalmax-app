...
// Versi Perbaikan: 2.2 - Logout Cleanup
// Perubahan:
// - Memperbarui `handleLogout` untuk mematikan semua listener aktif sebelum sign out.
...
export async function handleLogout() {
    try {
        // 1. Matikan semua listener aktif terlebih dahulu
        cleanupNotificationListener();
        // (Jika ada listener global lain, panggil fungsi cleanup-nya di sini)
        
        // 2. Baru lakukan sign out
        await signOut(auth);

        // 3. Muat halaman login (akan ditangani oleh onAuthStateChanged)
    } catch (error) {
