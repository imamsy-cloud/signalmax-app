// File: src/js/auth.js
// Deskripsi: Mengelola semua logika otentikasi pengguna, termasuk login, registrasi, dan manajemen sesi.
// Versi Perbaikan: 2.1
// Perubahan:
// - Menambahkan fungsi untuk memeriksa dan menampilkan banner interstitial setelah login.
...
import { loadScreen, setupNotificationListener } from './router.js';
 ...
 * Fungsi inti yang menentukan apakah pengguna login atau tidak, dan mengarahkan aplikasi.
 */
export function initAuthListener() {
    onAuthStateChanged(auth, async (user) => {
        ...
        if (user) {
            ...
            // PERBAIKAN: Panggil fungsi untuk cek banner setelah data pengguna dimuat
...
/**
 * [BARU] Memeriksa dan menampilkan banner interstitial sesuai aturan.
 */
async function checkAndShowInterstitialBanner() {
    try {
        ...
        if (!docSnap.exists() || !docSnap.data().interstitialActive) {
            return; // Banner tidak aktif, hentikan proses
        }
        ...
        if (lastShownTimestamp) {
            ...
            if (hoursSinceLastShown < frequencyHours) {
                return; // Belum waktunya tampil lagi
            }
        }

        // Jika semua kondisi terpenuhi, tampilkan banner
        ...
        if (modal && linkEl && imageEl && closeBtn) {
            ...
            if (bannerData.linkUrl && bannerData.linkUrl.startsWith('http')) {
                ...
                linkEl.target = '_blank'; // Buka di tab baru untuk link eksternal
            } else if (bannerData.linkUrl) {
                linkEl.href = '#'; // Hapus href agar tidak refresh
                linkEl.target = '';
                linkEl.onclick = (e) => {
                    e.preventDefault();
...
// ... Sisa file (handleGoogleLogin, handleEmailLogin, dll.) tetap sama ...
