...
// Versi: 7.0 (Sentralisasi Kompresi & Perbaikan Bug)
// Perubahan:
// - Menghapus fungsi compressImage lokal.
// - Mengimpor fungsi compressImage terpusat dari ui.js.

import * as api from '../api.js';
import { showMessage, setButtonLoading, openModal, closeModal, createPaginationControls, compressImage } from '../ui.js';
    ...
    const handler = async (e) => {
        ...
        try {
            ...
            if (data.image && data.image.size > 0) {
                ...
                const storageRef = api.ref(api.storage, `${collectionName}/${Date.now()}_${processedImage.name}`);
    ...
    tinymce.init({
        ...
        file_picker_callback: (callback, value, meta) => {
            ...
            input.onchange = async () => {
                ...
                try {
                    const storageRef = api.ref(api.storage, `content_images/${Date.now()}_${processedImage.name}`);
                    const snapshot = await api.uploadBytes(storageRef, processedImage);
...
function setupModal(modalId, collectionName, addL) {
    ...
    const handler = async () => {
        ...
        try {
            if (updateData.image) {
                const processedImage = await compressImage(updateData.image);
                const storageRef = api.ref(api.storage, `${collectionName}/${Date.now()}_${processedImage.name}`);
                const snapshot = await api.uploadBytes(storageRef, processedImage);
