    ...
    const handler = async (e) => {
        ...
        try {
            ...
            if (data.image && data.image.size > 0) {
                const storageRef = api.ref(api.storage, `events/${Date.now()}_${data.image.name}`);
                const snapshot = await api.uploadBytes(storageRef, data.image);
            ...
            showMessage("Gagal membuat event.", 3000, true);
        } finally {
