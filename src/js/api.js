...
// Versi: 5.3 (Add Badge Info to Posts)
// Perubahan:
// - [FIX] Memperbarui fungsi `submitPost` untuk menyertakan field `authorIsPremium`
//   dan `authorIsExpert`. Ini akan memungkinkan UI untuk menampilkan badge di postingan.
...
export {
    ...
    startAfter, startAt,
    ref, uploadBytes, getDownloadURL, deleteObject, onMessage
};
...
export async function submitPost(postData, currentUserData) {
    ...
    const dataToSave = {
        ...
        // --- PERBAIKAN DI SINI ---
        // Menambahkan status premium dan expert dari penulis postingan.
        authorIsPremium: currentUserData.isPremium || false,
        authorIsExpert: currentUserData.isExpert || false,
        // --- AKHIR PERBAIKAN ---
        createdAt: serverTimestamp(),
    ...
    batch.update(userRef, { 'stats.posts': increment(1) });
    await batch.commit();
}
...
export async function handleLikePost(postId, currentUserData) {
    ...
    if (!isLiked && postData.authorId !== auth.currentUser.uid) {
        ...
        addDoc(notifRef, {
            ...
            type: 'like'
        }).catch(err => console.error("Gagal mengirim notifikasi suka:", err));
    }
}
...
export async function submitComment(postId, text, currentUserData, parentCommentId = null) {
    ...
    try {
        ...
        console.error("Gagal mengirim notifikasi komentar:", error);
    }
}
...
async function getTotalLessonsInApp() {
    try {
        ...
        for (const courseDoc of coursesSnapshot.docs) {
            const chaptersQuery = query(collection(db, `courses/${courseDoc.id}/chapters`));
            const chaptersSnap = await getDocs(chaptersQuery);
            
            for (const chapterDoc of chaptersSnap.docs) {
                const lessonsQuery = query(collection(chapterDoc.ref, "lessons"));
